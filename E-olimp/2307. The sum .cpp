...
using namespace std;
   ...
   void build(long long v, long long left_pos, long long rigth_pos){
     ...
     void push(long long v, long long left_pos, long long rigth_pos){
       ...
       long long m=(left_pos+rigth_pos)/2;
       t[2*v].sum=(m-left_pos+1)*t[v].add;
       t[2*v+1].sum=(rigth_pos-m)*t[v].add;
   ...
   void setValue(long long v, long long left_pos, long long rigth_pos, long long l, long long r, long long value){

       if(l>r) return ;
       if(left_pos==l && rigth_pos==r){
           t[v].sum =(rigth_pos-left_pos+1)*value;
           t[v].add = value;
      //     cout<<left_pos<<" "<<rigth_pos<<" "<<l<<" "<<r<<endl;
           return;
       }
       ...
       push(v, left_pos, rigth_pos);
    //      cout<<left_pos<<" "<<rigth_pos<<" "<<l<<" "<<r<<" "<<m<<endl;
       setValue(2*v, left_pos, m, l, min(r,m), value);
       setValue(2*v+1, m+1, rigth_pos, max(l,m+1), r, value);
       t[v].sum=t[2*v].sum+ t[2*v+1].sum;
